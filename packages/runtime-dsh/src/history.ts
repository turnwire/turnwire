...
import { assistantId, compactText, imagePlaceholders, record, textContent, wireEventSchema, type WireEvent } from './mapper.js';
...
export function historyRecords(childId: string, events: WireEvent[], context: WireEvent[] = []): SubagentHistoryRecord[] {
  ...
  for (const event of events) {
    ...
    if (event.type === 'user/message') {
      const text = [textContent(d.content), imagePlaceholders(d.content)].filter(Boolean).join('\n');
      ...
      for (const value of Array.isArray(message.content) ? message.content : []) {
        ...
        const output = bounded(typeof block.content === 'string' ? block.content : [textContent(block.content), imagePlaceholders(block.content)].filter(Boolean).join('\n'));
