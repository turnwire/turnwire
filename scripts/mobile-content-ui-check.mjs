...
<header class="topbar"><button class="icon-button mobile-only" aria-label="Open session list">${icon(22)}</button><div class="breadcrumb">${icon(17)}<span>Host</span>${icon(12)}<strong>${long}</strong></div><div class="topbar-right"><span class="status waiting_approval"><span></span>Waiting for approval</span><button class="icon-button" aria-label="Connection settings">${icon(19)}</button></div></header>
...
<div class="inline-child"><section class="child-execution"><div class="child-execution-heading"><strong>Child execution</strong><span>${long}</span></div><div class="child-records"><article class="child-record"><details><summary>read · Complete</summary><pre>${long}</pre></details></article></div><small>${long}</small></section></div>
  ...
  for (const width of [320, 360, 390, 430]) {
    ...
    const result = await page.evaluate(() => {
      ...
      return {
        ...
        controls: bounds('.agent-item, .agent-more, .topbar > button, .topbar-right > button'),
        ...
        content: bounds('.topbar, .session-heading, .session-heading > *, .child-execution, .child-execution > *, .question-card, .agent-strip, .queued-strip'),
    ...
    await tool.locator('summary').click();
    console.log(JSON.stringify({ width, titleWidth: result.title.width, minControlHeight: Math.min(...result.controls.map(x => x.height)), minSummaryHeight: Math.min(...result.summaries.map(x => x.height)), inputFonts: result.fonts }));
  }
