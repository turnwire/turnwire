...
[[ -f "$DSH_ENV_FILE" ]] || { echo 'Create private config/dsh.env.json with the DSH environment first' >&2; exit 1; }
...
mkdir -p "$TURNWIRE_RUNTIME_DIR/dsh" "$TURNWIRE_HOME" "$TURNWIRE_CACHE_DIR"
