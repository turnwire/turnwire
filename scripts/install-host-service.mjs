...
const values = execFileSync('bash', ['-c', 'source "$1/scripts/host-paths.sh"; turnwire_resolve_paths || exit; shift; for key; do printf "%s\\0" "${!key}"; done', 'host-paths', root, ...keys], { env: cleanEnv }).toString().split('\0');
...
for (const file of [node, join(root, 'apps/daemon/dist/host-service.mjs'), paths.TURNWIRE_DSH_ENTRY, paths.TURNWIRE_DSH_ENV_FILE]) await access(file);
const bin = join(root, 'bin'); await mkdir(bin, { recursive: true });
for (const directory of new Set([paths.TURNWIRE_STATE_HOME, paths.TURNWIRE_DSH_HOME, paths.TURNWIRE_CONFIG_HOME, paths.TURNWIRE_CACHE_HOME])) { await mkdir(directory, { recursive: true, mode: 0o700 }); await chmod(directory, 0o700); }
await chmod(paths.TURNWIRE_DSH_ENV_FILE, 0o600);
const exports = keys.slice(0, 7).map(key => `if [ -z "\${${key}:-}" ]; then export ${key}=${shellQuote(paths[key])}; fi`).join('\n');
...
${keys.slice(0, 7).map(key => `Environment=${unitQuote(`${key}=${paths[key]}`)}`).join('\n')}
