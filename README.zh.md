...
本地使用：启动主机 → 打开本机 Web 或终端。远程使用：启动主机 → 配置远程入口 → 配对并访问。远程用户不必先安装原生客户端或在本机浏览器中完成任务；无界面主机可使用 `--no-open`，直接继续[远程访问配置](#配置远程访问按需)。
...
5. 本地用户现在即可在 Web 或终端中选择项目工作目录，创建会话并发送任务；远程用户可在完成远程配置和配对后再做此验证。能连接主机不代表模型密钥一定有效，实际任务才会验证模型服务访问。
...
| 客户端 | 如何使用 | 连接要求 |
| --- | --- | --- |
| Web / PWA（桌面或手机浏览器） | 本机打开启动器提供的 Web；远程打开配置好的 HTTPS Web 入口，也可添加到主屏幕 | 本机使用本机认证；远程客户端按[远程访问说明](#配置远程访问按需)完成配对 |
...
前置条件仅是执行主机已启动、具备主机管理权限，并准备好所选远程入口的配置；不需要先完成客户端选择。本地使用可跳过本节。远程用户配置入口后，用手机或其他设备的浏览器完成配对即可，无需另装原生应用。
