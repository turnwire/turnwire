...
四种方式都使用同一台执行主机，区别在于客户端如何连接它。

| 部署方式 | 需要准备 | 怎么用起来 |
| --- | --- | --- |
| 本地 | 执行主机和本机浏览器/终端，无需公网入口 | 运行 `npx turnwire@next --open`，使用已认证的本机连接 |
| 临时隧道 | 执行主机和可用的隧道服务 | 在远程访问设置中选择 localhost.run、cpolar 或 Cloudflare，开启临时访问，再配对远端客户端；地址变化后需要重新配对 |
| 命名隧道 | 已有的 Cloudflare 隧道、固定公开域名和私有隧道凭据 | 填写隧道名称、域名和凭据，启用后通过固定地址配对客户端 |
| 自部署 Relay | 自己的服务器、HTTPS 地址和 Relay 连接密钥 | [部署 Relay 与 PWA](docs/RELAY-INSTALL.zh.md)，在主机配置地址和密钥，再配对客户端；适合长期远程访问，也是推送的前提 |

Relay 和隧道不能替代执行主机。npm/npx 是安装选择，前台/后台及复用 DSH 是运行配置，不是额外的部署方式。主机准备和详细步骤见[快速开始](docs/QUICKSTART.zh.md)。
...
当前交付包含一次性配对、经设备凭据认证的每连接 ECDH 会话加密、分阶段连接恢复、持久审批收件箱、Web Push 和可配置的 TLS 局域网入口。首次部署使用独立的数据目录，并从主机创建客户端配对；配对凭据应私密保存。Relay 的连接路由仍在内存中，推送密钥与投递队列持久化。
