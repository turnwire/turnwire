...
- 补上缺失按钮后发现第二个问题：每行都能提交整组问题，把未回答的其他题静默结束，旧浏览器断言错误地认可了这个结果。现在整张卡片只有一个显式提交按钮，每题都有答案才能使用，点选项只修改草稿。`scripts/question-ui-check.mjs` 覆盖未填完/纯空白、输入法 Enter、可访问选中状态、禁用时保留控件、单选/多选、子代理列表临时失败、切换会话，以及 32 个子代理加 12 张审批卡片的手机区域高度。手机键盘各不相同，提交不能依赖物理 Enter 键。
...
- 已移除整回合自动折叠。把最后一条助手消息当成完整总结会隐藏前面的实质性回答，协议并没有这种保证。所有已收到的正文在结束及刷新后仍可见，工具详情保留独立展开。`scripts/history-ui-check.mjs` 检查后续回答出现后较早的实质性标题仍可见；`scripts/ui-check.mjs` 检查没有整回合折叠，同时保留工具详情展开断言。这修复的是已收到内容的呈现，不代表上游子代理一定已返回，也不能补写主代理遗漏的总结。
