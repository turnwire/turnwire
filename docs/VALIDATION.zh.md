...
- 提问曾经只能靠点选项或按 Enter 来回答：发送按钮只在多选题和完全没有选项的题上渲染，于是「有选项的单选题」会显示一个自由输入框、却没有任何可提交的东西；而手机键盘没有 Enter，这个输入框就成了死路。现在每一行可回答的题都带这个按钮，在该行有答案之前保持禁用。`scripts/question-ui-check.mjs` 要求夹具里三种题型各有一个按钮，往单选题那一行输入文字，要求按钮变为可用并成功作答，并要求记录里显示这段手写文本、另外两行显示「没有回答」；把旧条件改回去，它会报第一行按钮数为 0。
- 提问卡片既参差不齐又能被拖动，而且这个问题被报过两次。它的选项继承了共享按钮规则里的 `justify-content: center`，而每个选项又是一个单列隐式的 grid，于是每一行各自把自己那行标签和说明居中：夹具里的六个选项量出六条不同的左边缘（390px 下为 98、46、163、181、46、175）。卡片本身也没有声明 `min-width` 和换行，因此一个没有任何断行机会的选项标签——比如一条路径——把 454px 的按钮塞进了 278px 的卡片（卡片自身 `scrollWidth` 为 466，宽度 278），而用户手写的回答在 320px 下伸出卡片（252px 的行里装着 312px 的文本）。现在选项是一列显式的整行宽度，重新锚定了那个居中，并且提问里的所有文本都会在任意位置换行。`scripts/question-ui-check.mjs` 同时量 390px 和 320px：要求所有选项共用一条左边缘和同一个宽度、要求卡片的 `scrollWidth` 不超过自身宽度、要求已记录的回答放得进它那一行；把旧的选项尺寸改回去，它会报 "option labels do not share a left edge"，去掉换行则报卡片可拖动（390px 下期望 350、实测 440）。`tests/question-layout.test.ts` 为 `npm run check`（不跑浏览器）钉住同样这两条声明。
- 后台子代理原来只是一行字，没有入口。列表只显示标签、计时和当前步骤，于是子代理看起来卡住时最该看的东西——它自己上报的计划——在页面上根本打不开。现在这一行本身就是打开详情的控件：详情显示运行时对该子代理的描述（能否继续、嵌套时挂在哪个代理下）以及它的计划，计划是三列表格——标记、状态、步骤本身——每一行三列都对齐；没有上报计划的子代理会直接说明，而不是显示一张空表。列表只列出仍在运行的子代理；计划超过屏幕三分之一时在列表内滚动，而不是吃掉对话区；超过三行时提供展开其余。`turnwire agents` 打印的正是同一份详情，已在线上主机上对着真实的 DSH 子代理验证（四个子代理，其中一个有九步计划、一个没有计划、都是直接子级因此显示 `nesting level 1`）。`scripts/question-ui-check.mjs` 断言该行的 `aria-expanded`、三行计划及其状态、整列左边缘一致、列表不溢出，以及没有计划时的分支。
...
- Agent 等待回答的问题以前只是输入框上方的一个控件：回答发出后面板消失，聊天记录里既看不到问了什么、也看不到选了什么。现在问题是记录：`question.requested`/`question.resolved` 共用同一个 history key，一页会带上这一对，客户端投影把 `question` 行放在 Agent 提问的位置——单选题一次点击即回答，之后作为记录留存，`turnwire history` 与导出记录都会打印它。PWA 的实时事件过滤和 daemon 的历史投影都必须学会识别 question（它们此前只从 `sessionId`/`approval` 推导会话），这也是为什么即使投影支持了、那一行仍然不可见。包含提问的回合不再把步骤折叠掉。`scripts/ui-check.mjs` 断言卡片位于对话内、一次点击后变为 `answered`、显示所选标签并移除按钮；`tests/history.test.ts` 覆盖投影与导出。
- 贴住 chip 的模型面板一打开就被裁掉了：输入框卡片带着 `overflow: hidden`，于是面板位于卡片之上的部分——说明文字和模型下拉的上沿——被剪掉，那里由对话区覆盖。390px 下实测：面板盒 656–760，而输入框卡片盒是 688–808，卡片自身的 overflow 把 688 以上全部藏了起来。输入框卡片不再裁剪子元素（它内部的元素本来也不会画到圆角之外），`scripts/ui-model-check.mjs` 现在用 `elementFromPoint` 对面板的每个点做命中测试（从圆角内缩，避免把圆角当成遮挡），要求每一点都落在面板内部。把裁剪恢复后该检查报 `covered by section.conversation at 39,670`，修复后通过。
- 模型切换不再作为输入框上方的一块面板打开。以前点 chip 会把输入框往下推、把目录显示在别处；现在面板直接挂在 chip 上（与它的边缘对齐、向上展开，点外面或按 Esc 收起），在手机上也不会被键盘挡住。`scripts/ui-model-check.mjs` 会测量打开前后的输入框位置（必须不动）以及 chip 与面板之间的间距（必须紧贴）。
- 已经开始执行的排队提示可以被"取消掉"。线上 journal 显示了完整过程：seq 146415 的回合边界启动了那条等待中的提示，取消在 146423 才到达，而移除被记入了 journal——于是这一行从所有客户端消失，模型却仍在回答它。runtime 在把提示交出去之后仍保留 inbox 条目并接受对它的修改，所以"被接受"从来不是证据。主机现在会记录自己宣布过哪些提示已经开始：它们会离开 `session.queue`，而 `session.queueAction` 会以新增的 `QUEUE_ITEM_STARTED` 作答，而不是抹掉一条正在被回答的消息。Core 测试覆盖拒绝与队列过滤；`scripts/dsh-live-check.mjs` 在真实 Host 上复现了这一场景（队列不再列出它、迟到的移除返回 `QUEUE_ITEM_STARTED`）。
- DSH 从 `0.1.5-alpha.1`（源码 `5dda764`）升级到 `0.1.5-rc.1`（源码 `183f08e9`），也就是 npm `latest` tag 指向的版本。启动方式变了：`--patch` 现在是 profile 启动器的选项，而 `web` 别名不接受父级选项，因此所有启动都改为 `dsh --patch <file> --profile web …`（`package.json`、host service 以及两个检查脚本）。该版本还多了一个模型（`deepseek-flash`，现在是默认值），并给 Host 加了沙箱策略：`workspace-write` + `ask`，会话工作区或系统临时目录内的命令直接执行，其他路径触发审批。在旧版本上真实会话从不询问——线上 journal 里 135 次 bash 调用、0 次审批——所以这是第一个让手机端审批控件真正有活干的版本。`scripts/ui-model-check.sh` 已对新 Host 通过（认证、目录、选择、思考强度解析、重载），新增的 `scripts/dsh-live-check.mjs` 通过了真实回合、真实审批、其后的排队提示、runtime 队列投影以及主机派发记录。
- 已完成的回合现在只呈现结果。产生它的步骤——中间说明与所有工具调用——折叠成回答上方的一行 `过程 · N 步`，因为回来看会话的人要的是答案，而不是走过的路线。正在运行的回合仍然逐步展开（也正是在那里，折叠行上会显示最新的一条调用）；失败的回合保留步骤可见，失败不会被藏进折叠里。`scripts/ui-check.mjs` 断言回合结束后折叠恰好出现一次、默认关闭且工具调用在其中、回答仍然可见，并断言展开后工具调用仍是不套框的缩进形态。
- 回合运行期间发送的提示，以前会出现在它被*写下*的位置——也就是它等待的那段回答中间——因为 journal 在接受时就记录了它，之后没有任何事件标记它真正开始的时间。从线上主机读到的实据：每条排队提示都只被记录一次、带 `queued: true`，之后再无事件。主机现在会记录 runtime 开始执行等待中提示的时刻（回合边界处发出带 `queued: false` 的 `message.updated`，线上 journal 显示为相邻的 `idle`/`running` 事件对），共享投影会把这一行移到那个位置，因此每个客户端都把它放进真正运行它的回合，而不再标注为等待中。Core 测试覆盖该事件，投影测试覆盖排序，浏览器检查还断言等待中的提示最终进入消息流且不带排队标记。
- 手机上点「选择目录」没有反应，是因为主机仍在运行加入选择器之前的那次构建，而旧主机会用 `invalid` 这个 id 回应它不认识的方法。远程客户端根本无法匹配这个回复，于是丢弃它并一直等到自己的 35 秒超时——表现就是按钮死了。主机现在会用发起方自己的 `id` 回应无法解析的请求（`tests/core.test.ts`），选择器也改为在点击时就渲染、而不是等到主机回复，并带有读取状态和"主机较旧时可直接输入路径"的提示。`scripts/ui-check.mjs` 在禁用 service worker 的上下文里把目录列表延迟 1.2 秒，要求在 600 毫秒内看到选择器；该断言在旧实现上失败、在新实现上通过。
