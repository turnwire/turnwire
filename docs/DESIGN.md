...
Desktop: narrow session sidebar, wide readable conversation, compact device connection bar. Phone: conversation occupies the screen, with a deliberate session drawer. The identifying element is the continuity bar showing the actual connected host and selected session; it describes real state. Approvals sit next to the composer with the operation and a one-shot decision. Avoid presenting a settings dashboard as the main interface.
