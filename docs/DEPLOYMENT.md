...
Startup environment variables can also configure the connection:
...
Back up the resolved Turnwire config and state directories separately, plus DSH's state and attachments (`TURNWIRE_DSH_HOME`, defaulting to the `dsh/` subdirectory of Turnwire state). Config includes private `dsh.env.json` and `client.json`; state includes `state.db`, connection credentials and deployment records. Include SQLite WAL companion files; prefer stopping the managed host before a consistent backup, or use a SQLite online backup tool for SQLite databases. Do not copy a `state.db` that is being written on its own. Protect all backups as credentials. Runtime data and download caches are separate and are not substitutes for config/state/DSH backups. Store rejects old or incompatible databases with `UNSUPPORTED_STORAGE`; there is no migration. Use a separate empty database for this format and preserve old data offline. See the [maintenance guide](FIRST-UPGRADE.md). This version has no automatic event archiving or disk-quota management for unlimited history, so deployers should monitor disk usage.

A persistent Mac service can be managed by launchd running `node /absolute/path/turnwire/apps/daemon/dist/main.js`, with independent `TURNWIRE_STATE_HOME`, `TURNWIRE_CONFIG_HOME`, `TURNWIRE_DATA_HOME`, `TURNWIRE_CACHE_HOME` overrides and runtime/Relay parameters configured through `EnvironmentVariables`. The native App does not host the daemon itself, so closing its window does not kill tasks. The Mac must stay awake and online; software cannot keep a sleeping or powered-off Mac executing.
...
The headless host runs the same DSH adapter, daemon, CLI/TUI and encrypted remote-control protocol as the desktop installation. The install directory keeps `apps`, `packages` and source configuration; private configuration/state/runtime/cache use independent [XDG directories](XDG.md). `TURNWIRE_CONFIG_HOME`, `TURNWIRE_STATE_HOME`, `TURNWIRE_DATA_HOME` and `TURNWIRE_CACHE_HOME` override each directory separately. Otherwise each uses the corresponding absolute `XDG_*_HOME` plus `/turnwire`, defaulting to `~/.config/turnwire`, `~/.local/state/turnwire`, `~/.local/share/turnwire` and `~/.cache/turnwire`. `TURNWIRE_HOME` has been removed and is rejected. No checkout-local or other old layout is auto-detected or migrated. Build the release before copying it. `scripts/install-linux-host.sh` installs a checksum-verified Node runtime, locked production dependencies and the DSH runtime pinned by `config/dsh-runtime`, then registers the user service. Do not copy platform-specific `node_modules` from macOS to Linux.

Use `bash scripts/start-host.sh` for first-run credential setup. For manual installation, put the DSH environment mapping in the resolved Turnwire config directory as `dsh.env.json` with mode 0600, or explicitly select a private file with `TURNWIRE_DSH_ENV_FILE`; there is no checkout-local fallback. The file contains the `TURNWIRE_HARNESS_DEEPSEEK_API_KEY` environment variable. This private file is supplied on the destination, never included in release archives. The managed launcher passes its value only to DSH. Turnwire receives the short-lived loopback DSH connection URL; model values and URL tokens are redacted from service logs.
...
The installer generates the executable wrapper and `turnwire-host.service` for the current user, using the supplied install directory for application artifacts and the independently resolved XDG directories for private configuration, state, runtime data and cache. The service starts DSH on loopback, waits for its authenticated launch URL, then starts Turnwire. A failed child causes a supervised restart; shutdown drains Turnwire before DSH. Use `systemctl --user status turnwire-host`, `restart turnwire-host`, or `journalctl --user -u turnwire-host` to manage it. Enable lingering for startup at boot without an SSH login. Exiting TUI or SSH leaves the host service running.

Use `bin/turnwire remote` to select the existing Relay, then `bin/turnwire devices pair --name phone --qr` to pair the phone. Each host has its own identity, sessions and pairing; a pairing for another host does not switch automatically. Local daemon/DSH ports stay on loopback, and remote access goes through the encrypted Relay. The generic managed launcher also accepts `TURNWIRE_INSTALL_DIR`, `TURNWIRE_STATE_HOME`, `TURNWIRE_CONFIG_HOME`, `TURNWIRE_DATA_HOME`, `TURNWIRE_CACHE_HOME`, `TURNWIRE_DSH_HOME`, `TURNWIRE_DSH_ENTRY`, `TURNWIRE_DSH_ENV_FILE` and `TURNWIRE_DSH_PORT` when a different layout is required.

## Remote v2, notifications and LAN direct connection

Relay, host daemon and clients must all use the current v2 remote contract; old hosts, device credentials and pairing formats are not supported. Deploy Relay first, then the host and clients, without expecting mixed-version service. Keep the private deployment configuration for redeployment, but do not reuse an old host database: Store rejects it without migration. Create new v2 pairings with `turnwire devices pair --name phone --qr`; there is no device-upgrade command or PUT replacement endpoint. A new QR code is valid for 15 minutes and can only be enrolled once. See the [maintenance guide](FIRST-UPGRADE.md).
