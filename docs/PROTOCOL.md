...
Methods: `system.snapshot`, `session.create`, `session.resume`, `session.message`, `session.cancel`, `session.queue`, `session.queueAction`, `session.autoApprove`, `session.rename`, `session.archive`, `approval.decide`, `events.list`, `history.page`, `subagent.list`, `subagent.history`, `workspace.list`, `inbox.page`, `request.result`, `notifications.status`, `notifications.subscribe`, `notifications.unsubscribe`. Inputs are validated at the daemon boundary, and a request the host cannot parse or does not know is still answered under the requesting client's own `id` when one can be read — an unknown method is how an older host meets a newer client, and a reply addressed to nobody is one a remote client cannot match, so the caller would wait out its own timeout instead of seeing the error. Request IDs persist across restarts for mutations. An identical ID/payload returns the previous receipt. A changed payload conflicts. If the process died after reservation but before receipt persistence, Turnwire returns `OUTCOME_UNKNOWN` rather than replaying a potentially completed operation. This is an at-most-once submission boundary, not a claim of distributed exactly-once execution.
