...
Methods: `system.snapshot`, `session.create`, `session.resume`, `session.message`, `session.cancel`, `session.queue`, `session.queueAction`, `session.autoApprove`, `session.rename`, `session.archive`, `approval.decide`, `events.list`, `history.page`, `subagent.list`, `workspace.list`, `inbox.page`, `request.result`, `notifications.status`, `notifications.subscribe`, `notifications.unsubscribe`. Inputs are validated at the daemon boundary. Request IDs persist across restarts for mutations. An identical ID/payload returns the previous receipt. A changed payload conflicts. If the process died after reservation but before receipt persistence, Turnwire returns `OUTCOME_UNKNOWN` rather than replaying a potentially completed operation. This is an at-most-once submission boundary, not a claim of distributed exactly-once execution.
