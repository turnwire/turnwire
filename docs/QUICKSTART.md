...
## Choose from four deployment methods

| Deployment | Requirements and first steps |
| --- | --- |
| **Local** | Install and start the execution host below, then open its printed Web address on that same host. No public endpoint, domain, Relay or tunnel is needed. |
| **Temporary tunnel** | Start the host, ensure the selected provider's tunnel tool and outbound network access are available, then run `turnwire remote` and select temporary access. The default Cloudflare quick tunnel needs no owned domain; its public address can change after restart and is not a persistent entry point. |
| **Named tunnel** | Prepare an existing Cloudflare tunnel, a fixed hostname with DNS routed to that tunnel, its credentials JSON on the execution host, and `cloudflared`. Run `turnwire remote` and select the named Cloudflare provider, supplying the existing tunnel name, hostname and credentials-file path. Turnwire does not create the tunnel, DNS record or credentials for you. The hostname stays fixed, but access still depends on the host and tunnel process running. |
| **Self-deployed Relay** | Prepare your own reachable server and public HTTPS address (domain or supported public IP), SSH key/agent, administrator or passwordless sudo access, and public TCP 80/443. Run `turnwire deploy` and complete the form; see [Relay deployment](RELAY-INSTALL.md) for supported systems and steps. The server persistently serves Relay and the PWA at your own HTTPS endpoint; preserve its private configuration and push state. The execution host remains separate and must stay online. |

Local access remains available when remote access is enabled. For all remote methods, finish the [remote connection and pairing](#remote-connection-and-pairing) steps below. npx/global npm are **host installation** choices, external DSH is a **runtime** choice, systemd is **host process management**, and Demo is **development/testing**—none is another deployment mode.

Do not run multiple hosts against the same state or occupied ports. For a separate trial, isolate both config and state so it cannot overwrite the active local connection descriptor.

## Host installation: published npm preview (Linux / macOS)
...
### Host installation: npx foreground (no global installation)
...
### Host installation: global foreground (reusable command)
...
### Runtime configuration: reuse an authenticated external DSH
...
## Development/testing: source Demo without model credentials
...
## Host installation and process management: source-based Linux service
...
In the remote form, configure the temporary tunnel, existing named tunnel, or your self-deployed Relay selected above. For a Relay already deployed, enter its HTTPS endpoint and provide its connection credential privately. Run `turnwire remote status` to confirm the route is ready before pairing. Cloudflare is optional, not a requirement for local use or Relay. Open the remote HTTPS Web endpoint and pair using the one-use invitation, not the host's local connection token. Treat pairing codes, QR images and tunnel/Relay credentials as secrets. Confirm the actual phone connects and can resume a session; local status alone cannot verify a cellular network. For a persistent Relay PWA, install from your own stable HTTPS endpoint and retain the server's push database and configuration across restarts/updates. Bootstrap does not provision a VPS, configure DNS, open firewall ports or deploy public services without operator action. See [Relay deployment](RELAY-INSTALL.md).

Daemon and DSH remain loopback-only. No delegated approvals are enabled by bootstrap. Keep the host awake, protect private credentials, and back up Turnwire config and state plus DSH state/attachments, including an explicitly configured `TURNWIRE_DSH_HOME`. Use matching current Relay, host and client releases for pairing.
