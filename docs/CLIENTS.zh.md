...
| 在工作区中创建会话 | `new --cwd --runtime` | 相同命令 | 新建会话 / 文件夹选择器 |
...
后台子代理视图（`subagent.list`）、排队提示自己的三个操作、以及审批委托（`session.autoApprove`）已在 PWA 和 CLI/交互终端中提供。桌面端对应功能还没做，所以上表那几格是真实的缺口，而不是平台差异。被委托的会话也是唯一"批准之前没有人看过"的地方，所以桌面端必须先能显示这个状态，才谈得上提供开关。
...
手机始终显示连接栏，包括窄屏。`RemoteClient` 只有在与预期 Mac 完成一次加密 nonce 往返后才发布健康状态。它在前台时每 15 秒探测一次，5 秒后超时，并在浏览器回到前台时重建连接。V2 还会等待主机对最终确认的确认。Mac 在把设备标记为已连接之前，需要对其自身新发出的 challenge 收到加密确认；如果没有再次确认，其肯定状态会在 25 秒后过期。仅凭 Relay 就绪和已保存的配对凭据无法产生肯定的连接状态。这些是近期的观察结果，不保证未来的连通性或后台 iOS 执行。
