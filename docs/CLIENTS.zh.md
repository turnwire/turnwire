...
手机始终显示连接状态，包括窄屏：连接中、未验证或已断开时是连接栏——这些状态需要解释；一旦验证通过，就只剩一条细线，显示主机、往返延迟，点击可重新验证。连接失败所抱怨的问题一旦被重新连接解决，横幅会自行消失；而作为对用户操作的答复的错误则保留，直到被关闭或被新的结果取代。`RemoteClient` 只有在与预期 Mac 完成一次加密 nonce 往返后才发布健康状态。它在前台时每 15 秒探测一次，5 秒后超时；回到前台时先确认并复用仍然持有的 socket，而不是重建——只有确实已经不存在的 socket 才会重建，因此切标签页不再等于重连。V2 还会等待主机对最终确认的确认。Mac 在把设备标记为已连接之前，需要对其自身新发出的 challenge 收到加密确认；如果没有再次确认，其肯定状态会在 25 秒后过期。仅凭 Relay 就绪和已保存的配对凭据无法产生肯定的连接状态。这些是近期的观察结果，不保证未来的连通性或后台 iOS 执行。
