...
- **安全点。** 它会等待（默认 900s）直到没有会话处于 `running` 或 `waiting_approval`，然后才重启，因此重载永远不会打断一个回合。但**后台子代理不在这项检查里**：委派工具把活交给子代理后会立刻返回，父会话可能已经是空闲，而子代理还在跑，重启就会把它杀掉。要跑并行子代理前先停掉计时器（`systemctl --user stop turnwire-dev-reload.timer`），等子代理跑完再启动。运行时会报告在跑的子代理（快照里的 `busy`），所以等待也会覆盖它们。若子代理已经不在运行时视野里（例如手工重启之后），检查帮不上忙，此时就退回手工停掉计时器。
