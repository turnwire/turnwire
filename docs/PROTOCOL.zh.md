...
图片输入与只存引用的历史见[图片消息](IMAGES.zh.md)。`session.image {sessionId, attachmentId, offset?, limit?}` 是会话范围内的只读方法，返回 `{attachment, data, offset, nextOffset}`；偏移和长度按 base64 字符计，单块最多131072字符，由 SDK 有界拼接。runtime 的可选 `imageInput` 表示传输能力，不代表目录中的所有模型都支持视觉。
...
问题在历史页里是独立的一条记录：`question.requested` 与 `question.resolved` 共用一个 key（`<sessionId>:question:<id>`），因此一页会同时带上「问了什么」和「答了什么」，客户端投影可以把它们放在 Agent 提问的位置。
