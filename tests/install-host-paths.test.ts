...
const keys = ['TURNWIRE_STATE_HOME','TURNWIRE_CONFIG_HOME','TURNWIRE_DATA_HOME','TURNWIRE_CACHE_HOME','TURNWIRE_DSH_HOME','TURNWIRE_DSH_ENV_FILE','TURNWIRE_DSH_ENTRY','TURNWIRE_NODE','TURNWIRE_UNITS_DIR'];
async function fixture() {
  ...
  const paths = (extra: Record<string,string> = {}) => {
    const result = spawnSync('/bin/bash', ['-c', 'source "$1/scripts/host-paths.sh"; turnwire_resolve_paths || exit; shift; for key; do printf "%s\\0" "${!key}"; done','paths',root,...keys], { env: {...env,...extra}, encoding:'utf8' });
...
it('resolves fresh XDG defaults and ignores relative or empty XDG values', async () => {
  ...
  expect(p.TURNWIRE_STATE_HOME).toBe(`${f.env.HOME}/.local/state/turnwire`);
  expect(p.TURNWIRE_DSH_HOME).toBe(`${p.TURNWIRE_STATE_HOME}/dsh`);
  expect(p.TURNWIRE_DSH_ENV_FILE).toBe(`${f.env.HOME}/.config/turnwire/dsh.env.json`);
...
it.each(['state','runtime','dsh-state','config/dsh.env.json'])('ignores old installed marker %s without modifying it',async marker => {
  const f=await fixture(); const before=f.paths(); await mkdir(join(f.root,marker),{recursive:true});
  await mkdir(`${f.env.HOME}/.turnwire`); expect(f.paths()).toEqual(before);
});
it('honors namespaced overrides and ignores generic DSH aliases',async () => {
  const f=await fixture(); const baseline=f.paths();
  expect(f.paths({DSH_HOME:'/ignored',DSH_ENTRY:'/ignored',DSH_ENV_FILE:'/ignored'})).toEqual(baseline);
  const p=f.paths({TURNWIRE_STATE_HOME:`${f.root}/custom`,TURNWIRE_DSH_ENTRY:`${f.root}/external.js`,TURNWIRE_DSH_ENV_FILE:`${f.root}/key.json`});
  expect(p.TURNWIRE_CONFIG_HOME).toBe(baseline.TURNWIRE_CONFIG_HOME);
  expect(p.TURNWIRE_DSH_ENTRY).toBe(`${f.root}/external.js`); expect(p.TURNWIRE_DSH_ENV_FILE).toBe(`${f.root}/key.json`);
});
it.each(['/old',''])('rejects removed TURNWIRE_HOME (%j)', async value => {
  const f=await fixture();
  const result=spawnSync('/bin/bash',['-c','source "$1/scripts/host-paths.sh"; turnwire_resolve_paths','paths',f.root],{env:{...f.env,TURNWIRE_HOME:value},encoding:'utf8'});
  expect(result.status).not.toBe(0); expect(result.stderr).toContain('TURNWIRE_HOME has been removed');
});
it('writes quoted XDG unit and launcher with resolved environment without leaking credentials',async () => {
  ...
  for(const file of [p.TURNWIRE_NODE,p.TURNWIRE_DSH_ENTRY,p.TURNWIRE_DSH_ENV_FILE,`${f.root}/apps/daemon/dist/host-service.mjs`]) { await mkdir(resolve(file!,'..'),{recursive:true}); await writeFile(file!,''); }
