...
import { RemoteClient, randomSecret } from '@turnwire/sdk';
...
it('finishes encrypted initial replay and health verification with thousands of stored events', async () => {
  ...
  const pairing: Pairing = { v: 1, hostId: 'mac', clientId: 'phone', name: 'Phone', relayUrl: `ws://127.0.0.1:${relay.port}/relay`, token: randomSecret(), key: randomSecret() };
  ...
  expect((await phone.request<Snapshot>('system.snapshot')).cursor).toBe(4000);
...
it('still disconnects an authenticated phone that floods the relay', async () => {
  ...
  await connect({ kind: 'host', hostId: 'mac', token, clients: [{ id: 'phone', token: phoneToken }] });
  ...
  for (let i = 0; i < 1100; i++) phone.send(JSON.stringify({ type: 'payload', payload: { nonce: 'opaque', ciphertext: 'opaque' } }));
