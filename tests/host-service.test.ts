...
it('supervises DSH and daemon with isolated credentials, startup tokens and clean shutdown', async () => {
  ...
  await writeFile(join(root, 'config/dsh.env.json'), JSON.stringify({ NOVE_HARNESS_DEEPSEEK_API_KEY: key }), { mode: 0o600 });
  await writeFile(join(root, 'dsh.mjs'), `import {writeFileSync} from 'node:fs';
writeFileSync('dsh.json',JSON.stringify({hasKey:process.env.NOVE_HARNESS_DEEPSEEK_API_KEY==='${key}',hasRelayToken:!!process.env.TURNWIRE_RELAY_TOKEN}));
