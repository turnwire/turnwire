...
it('shares XDG config between isolated daemon and CLI while ignoring old data', async () => {
  ...
  await mkdir(join(root, '.turnwire'));
  await writeFile(join(root, '.turnwire/client.json'), 'old-layout-sentinel');
  ...
  try {
    ...
    expect(await readFile(join(root, '.turnwire/client.json'), 'utf8')).toBe('old-layout-sentinel');
    expect(await readdir(join(root, '.turnwire'))).toEqual(['client.json']);
