...
function client(result: unknown) { return { request: vi.fn().mockResolvedValue(result), subscribe: vi.fn(), close: vi.fn() } as unknown as TurnwireClient; }
it('passes cursor cuts intact and returns validated snapshot pages', async () => {
  ...
  expect(c.request).toHaveBeenCalledWith('subagent.history', args);
});
