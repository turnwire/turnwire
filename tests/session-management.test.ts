...
import { LocalClient, RemoteClient, conversation, transcriptMarkdown } from '@turnwire/sdk';
...
async function setup() {
  ...
  const device = { v: 2 as const, hostId: core.device.id, relayUrl, clientId: crypto.randomUUID(), token: randomSecret(), key: randomSecret(), name: 'Phone' }; core.store.addDevice(device); bridge.start();
  ...
  const session = await local.call('session.create', { runtimeId: 'demo', cwd: process.cwd(), title: 'Original title' });
...
it('renames, archives and restores the same session across local and encrypted clients without losing history', async () => {
  ...
  await local.call('session.message', { sessionId: session.id, text: 'Keep this history' });
  const count = conversation(core.store.events(0, 100), session.id).length;
  await phone.call('session.rename', { sessionId: session.id, title: ' Shared new title ' });
  expect((await local.call('system.snapshot')).sessions[0]?.title).toBe('Shared new title');
  await local.call('session.archive', { sessionId: session.id, archived: true });
  expect((await phone.call('system.snapshot')).sessions[0]?.archived).toBe(true);
  await expect(phone.call('session.message', { sessionId: session.id, text: 'Too early' })).rejects.toThrow('Unarchive this session');
  await expect(local.call('session.resume', { sessionId: session.id })).rejects.toThrow('Unarchive this session');
  await phone.call('session.archive', { sessionId: session.id, archived: false });
  expect(conversation(core.store.events(0, 100), session.id)).toHaveLength(count);
  await local.call('session.message', { sessionId: session.id, text: 'Continue' });
  expect(core.store.sessions()).toHaveLength(1);
  await expect(local.call('session.rename', { sessionId: session.id, title: '  ' })).rejects.toThrow();
});

it('refuses to archive a task that still needs an approval', async () => {
  const { local, session, core } = await setup();
  await local.call('session.message', { sessionId: session.id, text: '审批' });
  await expect(local.call('session.archive', { sessionId: session.id, archived: true })).rejects.toThrow('Stop the task');
  expect(core.store.session(session.id)?.archived).not.toBe(true);
  expect(core.store.approvals().some(a => a.status === 'pending')).toBe(true);
  await local.call('session.cancel', { sessionId: session.id });
  await local.call('session.archive', { sessionId: session.id, archived: true });
...
it('keeps tool input, error output, elapsed timestamps and safe Markdown fences on replay', () => {
  ...
  const markdown = transcriptMarkdown({ id: 's', title: 'Test', cwd: '/tmp', runtimeId: 'demo', runtimeSessionId: 's', status: 'idle', archived: false, autoApprove: false, createdAt: '', updatedAt: '' }, messages);
