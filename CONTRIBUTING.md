...
面向要改这个仓库的人。只想装上用，看 [README](README.md)；想先跑起来看 [从源码开发](docs/DEVELOPING.md)。

## 先读这两份

- [AGENTS.md](AGENTS.md) —— 工程约束：代码归属、能力对等、模型与凭据边界。它同时是自动化代理会读的规则，所以动手前先看。
- [docs/CLIENTS.md](docs/CLIENTS.md) —— 每项能力在各客户端的覆盖，以及"新增行为该放哪一层"。
...
接真实模型需要 DSH 与凭据，步骤见 [从源码开发](docs/DEVELOPING.md#接真实-dsh)；离线开发用 `TURNWIRE_RUNTIME=demo npm run dev` 就够了。
...
- 行为变更请同时更新 [docs/CLIENTS.md](docs/CLIENTS.md)。文档里的脚本名、仓库路径、CLI 命令、DSH 版本与凭据变量名由 [tests/docs.test.ts](tests/docs.test.ts) 机械核对，说明过时会直接让 CI 失败。
...
发现安全问题不要开公开 issue，走 [SECURITY.md](SECURITY.md)。
