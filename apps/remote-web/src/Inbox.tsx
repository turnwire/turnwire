...
import { call, type TurnwireClient } from '@turnwire/sdk';
...
export function Inbox({ client, cursor, connected, onOpen }: { client: TurnwireClient; cursor: number; connected: boolean; onOpen: (id: string) => void }) {
  ...
  async function load(older?: number) {
    const page = await call(client, 'inbox.page', { status: all ? 'all' : 'pending', before: older });
  ...
  useEffect(() => {
    ...
    void call(client, 'inbox.page', { status: all ? 'all' : 'pending' }).then(page => { if (active) { setItems(page.items); setBefore(page.nextBefore); } }).catch(error => { if (active) setError(errorText(error)); });
    void call(client, 'notifications.status').then(value => { if (active) setNotifications(value); }).catch(() => {});
    return () => { active = false; };
  ...
  async function enable() {
    ...
    setNotifications(await call(client, 'notifications.subscribe', methodSchemas['notifications.subscribe'].parse({ endpoint: value.endpoint, keys: value.keys })));
  }
  ...
  return <section className="inbox-view"><div className="inbox-heading"><div><h1>{t('inbox.title')}</h1><p>{t('inbox.subtitle')}</p></div><label><input type="checkbox" checked={all} onChange={e => setAll(e.target.checked)} />{t('inbox.includeHandled')}</label></div>
    ...
    <details className="request-result"><summary>{t('inbox.requestResult')}</summary><form onSubmit={e => { e.preventDefault(); void act(async () => setResult(JSON.stringify(await call(client, 'request.result', { requestId }), null, 2))); }}><label>{t('inbox.requestId')}<input value={requestId} onChange={e => setRequestId(e.target.value)} required /></label><button disabled={!connected || busy}>{t('inbox.check')}</button></form>{result && <pre>{result}</pre>}</details>
  </section>;
}
