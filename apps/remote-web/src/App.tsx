    ...
    <main>
      ...
      {showConnection ? <ConnectionView initial={connection?.kind === 'remote' && clientRef.current instanceof RemoteClient ? { kind: 'remote', code: encodePairing(clientRef.current.currentPairing) } : connection} connecting={loading} connected={!!snapshot} onConnect={saveConnection} onDisconnect={disconnect} onBack={() => setShowConnection(false)} />
        ...
        : <>
          ...
          {((turnRunning && prompt.trim() !== '') || pending.length > 0) && <div className="queue-area" role="status" aria-label={t('queue.aria')}>{pending.length > 0 && <div className="queued-strip">{pending.map(item => <div className="queued-item" key={item.id}><span className="queued-label">{t('queue.label')}</span><span className="queued-text">{item.text}</span></div>)}</div>}{turnRunning && prompt.trim() !== '' && <div className="queue-actions"><span className="queue-draft" title={prompt.trim()}>{prompt.trim()}</span><button type="button" className="steer-button" aria-label={t('composer.steerAria')} title={t('composer.steerTitle')} disabled={!connected || busy} onClick={() => submit(prompt, true)}>{t('composer.steer')}</button></div>}</div>}
