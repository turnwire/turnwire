...
export function App() {
  ...
  // Steering only means something while a turn runs; the default is always queue, and the control
  // resets when the turn ends so nobody steers by accident on the next one.
  const [steer, setSteer] = useState(false);
  const turnRunning = ['running', 'waiting_approval'].includes(session?.status ?? '');
  useEffect(() => { if (!turnRunning) setSteer(false); }, [turnRunning]);
