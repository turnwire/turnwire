...
export function App() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      let retry = true;
      try {
        const result = await call(client, 'subagent.list', { sessionId: selected });
        if (!active) return;
  ...
  const answerQuestion = (question: Question, answers: QuestionAnswerItem[]) => void perform(async c => { await call(c, 'question.answer', { questionId: question.id, answers }); });
  ...
  function setDelegated(sessionId: string, enabled: boolean) { void perform(async c => { await call(c, 'session.autoApprove', { sessionId, enabled }); }); }
  ...
  useEffect(() => {
    ...
    function bootstrap() { if (bootstrapping || unsubscribe || !active) return; bootstrapping = true; void call(c, 'system.snapshot').then(next => {
      if (!active) return;
      ...
      unsubscribe = c.subscribe(event => {
        ...
        if (nextState === 'connected' && wasConnected) void call(c, 'system.snapshot').then(value => { if (active) setSnapshot(previous => previous && previous.cursor > value.cursor ? previous : value); }).catch(() => {});
...
function ConversationRow({ row, running, agents, client, sessionId, connected, pendingQuestions, disabled, onAnswer }: { row: ConversationRowData; running: boolean; agents: SubagentView[]; client?: TurnwireClient; sessionId: string; connected: boolean; pendingQuestions?: Set<string>; disabled?: boolean; onAnswer?: (question: Question, answers: QuestionAnswerItem[]) => void }) {
  ...
  return <MessageBody message={message} lead={row.lead === true}>{(message.queued || message.steer) && <div className="message-tags">{message.queued && <span className="queued-chip">{t('message.queuedChip')}</span>}{message.steer && <span className="queued-chip">{t('message.steerChip')}</span>}</div>}<MessageImages client={client} sessionId={sessionId} images={message.images} /></MessageBody>;
}
