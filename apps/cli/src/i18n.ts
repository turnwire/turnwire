...
const en = {
  ...
  'command.send': 'Send text and/or images: send <session> "text" --image file.png (text before flags)',
  'option.image': 'local PNG/JPEG/WebP/GIF files; repeatable, max 2, 256 KiB each / 512 KiB total; text optional (max 16000 characters)',
  'images.count': 'Attach at most 2 images per message.',
  'images.local': '--image reads local files on this client, not URLs or remote host paths.',
  'images.file': '--image requires a regular local file.',
  'images.size': 'Image exceeds 256 KiB. Resize or compress it locally, then retry with the smaller file.',
  'images.total': 'Images exceed 512 KiB total. Resize or compress them locally before retrying.',
  'images.type': 'Unsupported image signature. Use a PNG, JPEG, WebP or GIF file (renaming the extension is not enough).',
  'images.empty': 'Provide text or at least one --image file.',
  'images.text': 'Text accompanying images must not exceed 16000 characters.',
  'images.unsupported': 'This runtime does not advertise image input transport support. This capability does not imply model vision support.',
  'images.attached': '[{count} image attachment(s)]',
  ...
  'command.inbox': 'Approvals and decision history',
  ...
  'option.steer': 'deliver at the next step of the running turn without aborting its current request',
  ...
  'remote.menu': '1 Temporary tunnel   2 Self-hosted Relay   3 Disable remote access\n4 Pair phone   5 Paired devices   6 Revoke device   7 Refresh status   8 One-click server deployment   9 LAN direct   10 Notifications   0 Back',
  ...
  'remote.choiceRange': 'Choose 0–10',
...
const zh: Record<keyof typeof en, string> = {
  ...
  'command.send': '发送文字和/或图片：send <session> "文字" --image file.png（文字放在选项前）',
  'option.image': '本机 PNG/JPEG/WebP/GIF 文件；可重复，最多 2 张，每张 256 KiB / 总计 512 KiB；文字可省略（最多 16000 字符）',
  'images.count': '每条消息最多附加 2 张图片。',
  'images.local': '--image 只读取此客户端的本机文件，不读取 URL 或远程主机路径。',
  'images.file': '--image 需要普通的本机文件。',
  'images.size': '图片超过 256 KiB。请在本机缩小尺寸或压缩后，使用较小的文件重试。',
  'images.total': '图片总计超过 512 KiB。请在本机缩小尺寸或压缩后重试。',
  'images.type': '不支持的图片签名。请使用 PNG、JPEG、WebP 或 GIF 文件（仅修改扩展名无效）。',
  'images.empty': '请提供文字或至少一个 --image 文件。',
  'images.text': '附带图片的文字不得超过 16000 字符。',
  'images.unsupported': '此运行时未声明支持图片输入传输。此能力不代表模型支持视觉。',
  'images.attached': '[{count} 张图片附件]',
  'command.resume': '恢复已持久化的会话',
  'command.stop': '取消当前代理回合',
  'command.result': '响应丢失后查询命令结果',
  'command.inbox': '审批中心与处理记录',
  ...
  'option.steer': '在当前回合的下一步送达，不中止当前请求',
  ...
  'remote.menu': '1 临时隧道   2 自托管 Relay   3 关闭远程访问\n4 配对手机   5 已配对设备   6 撤销设备   7 刷新状态   8 一键部署服务器   9 局域网直连   10 通知   0 返回',
  'remote.choose': '选择 > ',
...
const enErrors = {
  ...
  IMAGE_INPUT_UNSUPPORTED: 'This runtime does not support image input transport',
  IMAGE_NOT_FOUND: 'Image attachment not found',
  INVALID_IMAGE: 'Invalid image; use a supported PNG, JPEG, WebP or GIF within the size limits',
  IMAGE_QUEUE_EDIT_UNSUPPORTED: 'Queued image messages cannot be edited; remove the message and send it again',
...
const zhErrors: Record<keyof typeof enErrors, string> = {
  ...
  IMAGE_INPUT_UNSUPPORTED: '此运行时不支持图片输入传输',
  IMAGE_NOT_FOUND: '找不到图片附件',
  INVALID_IMAGE: '图片无效；请使用大小限制内的 PNG、JPEG、WebP 或 GIF',
  IMAGE_QUEUE_EDIT_UNSUPPORTED: '无法编辑排队中的图片消息；请删除消息后重新发送',
