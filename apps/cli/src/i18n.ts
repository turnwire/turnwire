...
const en = {
  ...
  'command.dirs': 'List or create host folders a session could start in',
  'option.mkdir': 'Create one folder in [path] (defaults to host home) and list it',
  ...
  'command.agents': 'Watch the background agents a session has running',
  ...
  'command.questions': 'List questions the running agent is waiting on',
  'command.answer': 'Answer a pending question with option labels, or --text for a written answer',
  'command.autoApprove': 'Grant this session approvals as they arrive, until turned off',
  ...
  'autoApprove.on': 'Approvals are now granted for you',
  'autoApprove.off': 'Approvals wait for you again',
  'questions.none': 'No question is waiting',
  'option.before': 'load the page before this cursor',
  'option.answerText': 'a written answer sent as the custom reply',
  'option.off': 'turn it back off instead',
  ...
  'agents.detailOption': 'Show this child’s execution history',
  'agents.cursorOption': 'Read records after this cursor (-1 starts at the beginning)',
  'agents.integerError': '{flag} must be an integer from {minimum} to {maximum}.',
  'agents.detailRequired': '--before, --cursor and --limit require --detail <child>.',
  'agents.activityRunning': 'running',
  'agents.activityInactive': 'inactive',
  'agents.roleUser': 'user',
  'agents.roleAssistant': 'assistant',
  'agents.roleTool': 'tool',
  'agents.roleError': 'error',
  'agents.empty': 'No background agent is running under this session',
  ...
  // The plan under one agent: the same detail the phone opens a row to show.
  'agents.plan': 'plan:',
  'agents.noPlan': 'no plan reported',
  'agents.modeOneShot': 'one-shot',
  'agents.modeContinuable': 'can continue',
  'agents.under': 'under {label}',
  'agents.depth': 'nesting level {depth}',
...
const zh: Record<keyof typeof en, string> = {
  ...
  'command.dirs': '列出或创建主机上可以作为会话目录的文件夹',
  'option.mkdir': '在 [path]（默认为主机主目录）中创建一个文件夹并列出其内容',
  ...
  'command.questions': '列出正在运行的 Agent 在等回答的问题',
  'command.answer': '用选项标签回答待处理的问题，或 --text 写一条回答',
  'command.autoApprove': '这个会话的审批一到就自动批准，直到关掉',
  ...
  'autoApprove.on': '现在会代你批准',
  'autoApprove.off': '审批重新等你处理',
  'questions.none': '没有等待回答的问题',
  'option.before': '加载此游标之前的页',
  'option.answerText': '作为自定义回答发送的文字',
  'option.off': '改为关掉它',
  ...
  'models.failure': '\n不可用：{name} ({id}) — {message}',
  'agents.detailOption': '显示此子代理的执行记录',
  'agents.cursorOption': '读取此游标之后的记录（-1 表示从头开始）',
  'agents.integerError': '{flag} 必须是 {minimum} 到 {maximum} 之间的整数。',
  'agents.detailRequired': '--before、--cursor 和 --limit 需要与 --detail <child> 一起使用。',
  'agents.activityRunning': '执行中',
  'agents.activityInactive': '非活跃',
  'agents.roleUser': '用户',
  'agents.roleAssistant': '助手',
  'agents.roleTool': '工具',
  'agents.roleError': '错误',
  'agents.empty': '这个会话下没有正在跑的子代理',
  'agents.seconds': '{value} 秒',
  'agents.minutes': '{minutes} 分 {seconds} 秒',
  'agents.steps': '{done}/{total} 项待办',
  'agents.current': '{done}/{total} 项待办 · {content}',
  'agents.plan': '计划：',
  'agents.noPlan': '没有上报计划',
  'agents.modeOneShot': '一次性',
  'agents.modeContinuable': '可继续',
  'agents.under': '隶属于 {label}',
  'agents.depth': '嵌套层级 {depth}',
...
const enErrors = {
  ...
  QUEUE_ITEM_GONE: 'That message is no longer waiting to be sent',
  QUEUE_ITEM_STARTED: 'That prompt has already started running and cannot be taken back',
  QUESTION_EXPIRED: 'That question was already answered or has expired',
  WORKSPACE_UNREADABLE: 'The host cannot read that folder; choose another one',
...
const zhErrors: Record<keyof typeof enErrors, string> = {
  ...
  QUEUE_ITEM_GONE: '这条消息已经不在排队中了',
  QUEUE_ITEM_STARTED: '这条提示已经开始执行，无法撤回',
  QUESTION_EXPIRED: '这个问题已经回答过或已过期',
  WORKSPACE_UNREADABLE: '主机无法读取该目录，请选择其他目录',
