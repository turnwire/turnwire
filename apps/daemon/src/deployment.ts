...
export class DeploymentController implements DeploymentAccess {
  private current:DeploymentStatus;private operation?:Promise<void>;private aborter?:AbortController;private disposed=false;
  constructor(private store:Store,private runner:DeploymentRunner,private remote:RemoteAccess,private activity:HostActivity) {
  ...
  start(value:unknown) { return this.activity.run(() => this.startAdmitted(value)); }
  private startAdmitted(value:unknown) {
    if(this.disposed)throw new Error('The deployment service is shutting down');
    ...
    this.operation=this.activity.run(()=>Promise.resolve().then(async()=>{
  ...
  async close() {this.disposed=true;this.aborter?.abort();await this.operation;}
}
