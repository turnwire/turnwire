...
export class DirectController {
  ...
  constructor(private core: TurnwireCore, private origin: () => string | undefined, private allowed: () => boolean) { this.configuration = core.store.setting<DirectConfiguration>('direct-preferences') ?? { enabled: false }; }
  ...
  async close() { ++this.generation; await this.operation; await this.release(); this.state = 'off'; }
}
