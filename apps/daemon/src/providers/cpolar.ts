...
export async function cpolarBinary(options: TunnelOptions): Promise<string> {
  ...
  if (process.platform !== 'darwin' || !digests[process.arch]) throw new Error('Install cpolar and add it to PATH, or set TURNWIRE_CPOLAR_PATH');
  options.progress('First run: downloading and verifying the cpolar component…');
  ...
  if (!response.ok || !response.body) throw new Error('cpolar component download failed; check the network');
...
export function cpolarAddress(line: string): string | undefined {
  if (/authentication failed|failed to authenticate|authToken auth failed|invalid authtoken|ERR_CPOLAR|认证失败|验证失败/i.test(line)) throw new Error('cpolar authentication or channel startup failed; check the token, account quota, and network');
...
export async function startCpolarTunnel(options: TunnelOptions) {
  if (!options.token) throw new Error('First cpolar use: enter the account Auth Token');
  ...
  options.progress('Connecting to the cpolar channel in mainland China…');
  // 3.3.18 treats inspect-addr=false as :0; bind explicitly to loopback instead.
  try { return await processTunnel(options, binary, ['http', '-config=' + config, '-daemon=off', '-dashboard=off', '-processMode=single', '-inspect-addr=127.0.0.1:0', '-proto=https', '-log=stdout', '-log-level=INFO', '-region=cn', '-tunnelName=turnwire', `127.0.0.1:${options.port}`], cpolarAddress, 'cpolar could not establish the channel; check the token, the free account online-process quota, and the network', () => rm(runDirectory, { recursive: true, force: true })); }
